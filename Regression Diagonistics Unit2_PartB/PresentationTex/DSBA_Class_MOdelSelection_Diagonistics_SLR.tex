...
\qBrd[4.6in]{almond!70}{ The AIC Criterion for a General model is:\\ \DBX{AIC =\HLTW{2\HLTY{ p } {  -  } 2\times \text{Log-Likelihood}\left(\text{Model}\right)}.   } Here \HLTY{ p }: denotes the model dimension/ model complexity.
}
...
\qBrd[4.6in]{almond!70}{ The BIC Criterion for a General model is:\\ \DBX{BIC =\HLTW{\HLTY{ p }\log(n)- 2\times \text{Log-Likelihood}\left(\text{Model}\right).}   } Here \HLTY{ p }: denotes the model dimension/ model complexity.
}
...
\qBoxCol{teal!30}{
The observed data: $\{y_i, \bx_i\}_{i=1}^n $.  According to the model: \HLTY{Y_i= \bx^T_i\bbeta+\varepsilon_i}
}
...
\TransitionFrame[babyblue]{\Large  A Template of a Statistical Hypothesis Testing}
...
\qBrd{blue!20}{Null is Denoted as $H_0$, Alternative is denoted as $H_1$ or $H_a$}
